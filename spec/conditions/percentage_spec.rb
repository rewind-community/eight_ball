...
RSpec.describe EightBall::Conditions::Percentage do
  ...
  describe 'satisfied?' do
    ...
    it 'should decorrelate an id across different flags (different salt)' do
      value = 'org-123'
      buckets = %w[FlagA FlagB FlagC].map do |flag|
        Integer(Digest::SHA256.hexdigest("#{flag}:#{value}")[0, 8], 16) % 100
      end
      # Not a strict guarantee, but the canonical salt makes these independent;
      # for these three fixed names they are not all equal.
      expect(buckets.uniq.size).to be > 1
    end
  end
